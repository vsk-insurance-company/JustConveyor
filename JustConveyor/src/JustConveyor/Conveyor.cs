...
namespace JustConveyor
{
    public sealed class Conveyor : ConveyorContract
    {
        ...
        public ConveyorContract WithMetricsService(ServiceSettings service = null)
        {
            mServiceSettings = service ?? new ServiceSettings
            {
        ...
        public ConveyorContract WithFinalizer(Action<Package, TransferingContext> finalizer)
        {
            if (finalizer == null) throw new ArgumentNullException(nameof(finalizer));
            foreach (var pipelineBlueprintWrapper in BlueprintsMap)
                pipelineBlueprintWrapper.Value.Finalizers.Add(finalizer);
            return this;
        }
